...
public class DubboRegisterPlugin extends AbstractMojo {
    ...
    private Map<String,String> getKeyPrefix(){
        ...
        if(StringUtils.isBlank(dubboPkg))
            throw new RuntimeException("parameter serviceKeyPrefix and dubboPkg must has one");
        String dubboIntefacePath = dubboPkg.replace(".", File.separator);
        String path = project.getBuild().getOutputDirectory();
        ...
        for (File f : interfaceDir.listFiles()) {
            ...
            if (clzName.endsWith(".class")) {
                ...
                clzMap.put(clzSimpleName, dubboPkg + "." + clzSimpleName);
            }
        }
        return clzMap;
    }
